...
use sqlx::{PgPool};
use ts_rs::TS;
use api_lib::handshake::{DbRange, Direction};
use api_lib::query_db_range;
use crate::middleware::JwtAuth;

#[derive(Clone, Debug, Serialize, Deserialize, TS)]
#[ts(export, export_to = "../src/lib/handshake/schema/User.ts")]
pub struct User {
  username: String,
  email: String,
  is_verified: bool,
...
impl User {
  ...
  pub async fn get_all(db: &PgPool, range: DbRange) -> Option<Vec<Self>> {
    query_db_range!(Self, range, db, r#"SELECT username, email, is_verified, last_login FROM "user""#, "username", 0.01)
  }
